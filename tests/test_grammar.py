...
def test_system_from_kappa():
    ...
    system = kappa.system(system_str)
    n = system.variables["n"].evaluate()
    assert n == 300

    assert system.variables["g_on"].evaluate(system) == 0.003
    assert len(system.mixture.components) == 300
    assert system.count_observable("A_total") == 300

    for i in range(1, n):
        system.wait()
        system.act()

        assert system.count_observable("A_total") == n
        assert system.count_observable("A_u") == i
        assert system.count_observable("B_u") == n
        assert system.count_observable("A_p") == n - i
        assert system.count_observable("pairs") == n
